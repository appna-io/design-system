...
export default defineConfig({
  ...
  splitting: false,
  target: 'es2022',
  external: ['react', 'react-dom', '@apx-ui/engine', '@apx-ui/tokens'],
  async onSuccess() {
    const src = 'src/styles/reset.css';
    const dest = 'dist/styles/reset.css';
    mkdirSync(dirname(dest), { recursive: true });
    copyFileSync(src, dest);
    // Only the main runtime entry needs the client directive; tailwind-preset is pure data.
