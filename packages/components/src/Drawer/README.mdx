...
<ExampleBlock for="Overview" title="Overview — trigger, header, body, and footer" />

## When to use

- Mobile navigation menus (`side="start"`, `size="sm"`).
- Filter / inspector panels next to the main content (`side="end"`).
...
- **Side semantics**: `side` takes either grammar. `start` / `end` are **logical** — resolved
  against the ambient direction, so `end` is the right edge in LTR and the left edge in RTL.
  `left` / `right` / `top` / `bottom` are **physical** and never flip. Prefer the logical pair for
  anything anchored to a trigger (a cart at `end` opens on the same side as the basket button in
  both directions); reach for a physical value only when you mean a fixed edge. There is no
  logical vertical pair — the block axis does not flip with `dir`, so `top` / `bottom` are already
  correct. The inner border uses logical axes (`border-s` / `border-e`) either way.
