...
Target: **< 4 KB gz** (excluding `<Menu>` / `<Tooltip>` / `<Popover>` / `<Button>` peers, which
are externalized). Roving-tabindex logic is component-local; `ResizeObserver` overflow only
activates when `overflow="menu"` is set.
