'use client';

import { forwardRef } from '@apx-ui/engine';
import { Calendar as CalendarIcon, X as ClearIcon } from 'lucide-react';
import {
  useCallback,
  useEffect,
  useId,
  useMemo,
  useState,
  type FocusEvent,
  type KeyboardEvent,
} from 'react';

import { Calendar } from '../Calendar/Calendar';
import { formatIsoDate } from '../Calendar/helpers/dateMath';
import { formatDate, parseDateFormat } from '../Calendar/helpers/parseDateFormat';
import { useSlotClass as useCalendarSlotClass } from '../Calendar/helpers/useSlotClass';
import {
  datePickerApplyBarRecipe,
  datePickerPopoverContentRecipe,
  datePickerPresetItemRecipe,
  datePickerPresetListRecipe,
  datePickerTriggerRecipe,
} from '../Calendar/Calendar.recipe';
import { Input } from '../Input/Input';
import { Popover } from '../Popover';

import type { DatePickerPreset, DatePickerProps } from './DatePicker.types';

const DEFAULT_FORMAT = 'MM/dd/yyyy';

/**
 * Single-date picker. An `<Input>` for typed entry composed with a `<Popover>` hosting a
 * `<Calendar>`. The two surfaces stay in sync:
 *
 *   - **Calendar click** → selection commits → input mirrors the formatted value → popover
 *     closes (when `closeOnSelect`).
 *   - **Input commit** (Enter / blur) → `parse(formatted)` resolves a Date → selection updates
 *     → calendar's `month` jumps to the new value.
 *
 * The component is fully controllable (`value` + `open`) and fully uncontrolled (`defaultValue`
 * + `defaultOpen`). A hidden `<input type="hidden">` carries the ISO-8601 (`YYYY-MM-DD`)
 * payload so form posts get a locale-free, unambiguous value regardless of `format`.
 */
function DatePickerImpl(
  props: DatePickerProps,
  ref: React.ForwardedRef<HTMLDivElement>,
) {
  const {
    value: valueProp,
    defaultValue,
    onChange,
    format = DEFAULT_FORMAT,
    parse,
    placeholder,
    inputProps,
    showTriggerIcon = true,
    clearable = true,
    min,
    max,
    isDateDisabled,
    locale = 'en-US',
    weekStartsOn,
    numberOfMonths = 1,
    showWeekNumbers,
    presets,
    variant = 'solid',
    size = 'md',
    color = 'primary',
    name,
    required,
    disabled,
    invalid,
    readOnly,
    translations,
    open: openProp,
    defaultOpen,
    onOpenChange,
    closeOnSelect = true,
    'aria-label': ariaLabel,
    'aria-labelledby': ariaLabelledBy,
    'aria-describedby': ariaDescribedBy,
    className,
    style,
    footer,
  } = props;

  /* ---------------- value (controlled/uncontrolled) ----------------------- */

  const isValueControlled = valueProp !== undefined;
  const [internalValue, setInternalValue] = useState<Date | null>(defaultValue ?? null);
  const value = isValueControlled ? valueProp ?? null : internalValue;

  const setValue = useCallback(
    (next: Date | null) => {
      if (!isValueControlled) setInternalValue(next);
      onChange?.(next);
    },
    [isValueControlled, onChange],
  );

  /* ---------------- open (controlled/uncontrolled) ------------------------ */

  const isOpenControlled = openProp !== undefined;
  const [internalOpen, setInternalOpen] = useState<boolean>(defaultOpen ?? false);
  const open = isOpenControlled ? openProp ?? false : internalOpen;
  const setOpen = useCallback(
    (next: boolean) => {
      if (!isOpenControlled) setInternalOpen(next);
      onOpenChange?.(next);
    },
    [isOpenControlled, onOpenChange],
  );

  /* ---------------- typed-input mirror ----------------------------------- */

  const [text, setText] = useState<string>(() =>
    value ? formatDate(value, format, locale) : '',
  );

  // Keep the typed string in sync when `value` changes externally.
  useEffect(() => {
    setText(value ? formatDate(value, format, locale) : '');
  }, [value, format, locale]);

  const parseInput = useCallback(
    (raw: string): Date | null => {
      const trimmed = raw.trim();
      if (!trimmed) return null;
      if (parse) return parse(trimmed);
      return parseDateFormat(trimmed, format, locale);
    },
    [parse, format, locale],
  );

  const commitText = useCallback(() => {
    const parsed = parseInput(text);
    if (!parsed) {
      // Invalid — revert to whatever `value` is so the input never holds garbage.
      setText(value ? formatDate(value, format, locale) : '');
      return;
    }
    setValue(parsed);
  }, [parseInput, text, value, format, locale, setValue]);

  /* ---------------- handlers --------------------------------------------- */

  const handleInputChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => setText(e.target.value),
    [],
  );

  const handleInputKeyDown = useCallback(
    (e: KeyboardEvent<HTMLInputElement>) => {
      if (e.key === 'Enter') {
        e.preventDefault();
        commitText();
        setOpen(false);
      } else if (e.key === 'ArrowDown' && !open) {
        e.preventDefault();
        setOpen(true);
      } else if (e.key === 'Escape' && open) {
        e.preventDefault();
        setOpen(false);
      }
    },
    [commitText, open, setOpen],
  );

  const handleInputBlur = useCallback(
    (e: FocusEvent<HTMLInputElement>) => {
      inputProps?.onBlur?.(e);
      commitText();
    },
    [inputProps, commitText],
  );

  const handleCalendarChange = useCallback(
    (next: unknown) => {
      const date = next instanceof Date ? next : null;
      setValue(date);
      if (closeOnSelect) setOpen(false);
    },
    [setValue, closeOnSelect, setOpen],
  );

  const handleClear = useCallback(
    (e: React.MouseEvent) => {
      e.stopPropagation();
      setValue(null);
      setText('');
    },
    [setValue],
  );

  /* ---------------- presets ---------------------------------------------- */

  const handlePresetClick = useCallback(
    (preset: DatePickerPreset) => {
      const next = preset.date();
      setValue(next);
      if (closeOnSelect) setOpen(false);
    },
    [setValue, closeOnSelect, setOpen],
  );

  /* ---------------- accessibility ---------------------------------------- */

  const reactId = useId();
  const dialogId = `datepicker-${reactId}`;

  /* ---------------- recipes ---------------------------------------------- */

  const triggerClass = useCalendarSlotClass('datepicker.trigger', datePickerTriggerRecipe, {
    size,
  });
  const popoverContentClass = useCalendarSlotClass(
    'datepicker.popover',
    datePickerPopoverContentRecipe,
    { withPresets: Boolean(presets && presets.length > 0) },
  );
  const presetListClass = useCalendarSlotClass(
    'datepicker.presetList',
    datePickerPresetListRecipe,
    {},
  );
  const presetItemClass = useCalendarSlotClass(
    'datepicker.presetItem',
    datePickerPresetItemRecipe,
    {},
  );
  const applyBarClass = useCalendarSlotClass(
    'datepicker.applyBar',
    datePickerApplyBarRecipe,
    {},
  );

  /* ---------------- render ----------------------------------------------- */

  const triggerIcon = useMemo(() => {
    if (value && clearable) {
      return (
        <button
          type="button"
          onClick={handleClear}
          className={triggerClass}
          aria-label="Clear"
          tabIndex={-1}
        >
          <ClearIcon aria-hidden size={iconSize(size)} />
        </button>
      );
    }
    if (showTriggerIcon) {
      return (
        <Popover.Trigger asChild>
          <button
            type="button"
            className={triggerClass}
            aria-label="Open calendar"
            aria-haspopup="dialog"
            aria-expanded={open}
            aria-controls={dialogId}
            disabled={disabled || readOnly}
            tabIndex={-1}
          >
            <CalendarIcon aria-hidden size={iconSize(size)} />
          </button>
        </Popover.Trigger>
      );
    }
    return null;
  }, [
    value,
    clearable,
    showTriggerIcon,
    triggerClass,
    handleClear,
    size,
    open,
    dialogId,
    disabled,
    readOnly,
  ]);

  return (
    <div
      ref={ref}
      className={['inline-flex flex-col gap-1', className].filter(Boolean).join(' ')}
      style={style}
    >
      <Popover open={open} onOpenChange={setOpen}>
        <Popover.Trigger asChild>
          <Input
            {...(inputProps ?? {})}
            value={text}
            onChange={handleInputChange}
            onKeyDown={handleInputKeyDown}
            onBlur={handleInputBlur}
            placeholder={placeholder ?? format}
            size={size}
            variant={mapVariantToInput(variant)}
            color={color}
            disabled={disabled}
            invalid={invalid}
            readOnly={readOnly}
            required={required}
            aria-label={ariaLabel}
            aria-labelledby={ariaLabelledBy}
            aria-describedby={ariaDescribedBy}
            aria-haspopup="dialog"
            aria-expanded={open}
            aria-controls={dialogId}
            rightIcon={triggerIcon}
          />
        </Popover.Trigger>

        <Popover.Content
          id={dialogId}
          role="dialog"
          aria-label={ariaLabel ?? 'Calendar'}
          className={popoverContentClass + ' p-0'}
        >
          {presets && presets.length > 0 ? (
            <div className={presetListClass} role="list">
              {presets.map((preset) => (
                <button
                  key={preset.id}
                  type="button"
                  role="listitem"
                  className={presetItemClass}
                  onClick={() => handlePresetClick(preset)}
                >
                  {preset.label}
                </button>
              ))}
            </div>
          ) : null}

          <div className="flex flex-col">
            <Calendar
              mode="single"
              value={value}
              onChange={handleCalendarChange}
              min={min}
              max={max}
              isDateDisabled={isDateDisabled}
              locale={locale}
              weekStartsOn={weekStartsOn}
              numberOfMonths={numberOfMonths}
              showWeekNumbers={showWeekNumbers}
              translations={translations}
              variant={variant}
              size={size}
              color={color}
            />
            {footer ? <div className={applyBarClass}>{footer}</div> : null}
          </div>
        </Popover.Content>
      </Popover>

      {name ? (
        <input
          type="hidden"
          name={name}
          value={value ? formatIsoDate(value) : ''}
          required={required}
        />
      ) : null}
    </div>
  );
}

DatePickerImpl.displayName = 'DatePicker';

function iconSize(size: 'sm' | 'md' | 'lg'): number {
  if (size === 'sm') return 14;
  if (size === 'lg') return 18;
  return 16;
}

/**
 * Translate the Calendar's variant axis to the Input's. They share three of four values
 * directly; `soft` doesn't exist on Input (we use `outline`) and `minimal` maps to `ghost`.
 */
function mapVariantToInput(
  variant: 'solid' | 'outline' | 'soft' | 'minimal',
): 'solid' | 'outline' | 'ghost' {
  if (variant === 'minimal') return 'ghost';
  if (variant === 'soft') return 'outline';
  return variant;
}

export const DatePicker = forwardRef(DatePickerImpl);
