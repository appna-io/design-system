...
Menu consumes the entire overlay surface — no new engine primitives requested:

| Primitive | Used for |
|---|---|
| `usePosition` | Position Content (and SubContent) against trigger; supports virtual-element anchoring for context-menu mode. |
| `<Portal>` | Render Content outside the DOM tree so z-index + overflow clip-paths can't trap the menu. |
| `useEscapeStack` | Esc closes innermost menu first (sub before root). |
| `useOutsideClick` | Pointer-down outside trigger + content closes. |
| `useControllableState` | `open` / `defaultOpen` / `onOpenChange`, plus CheckboxItem's `checked` / `onCheckedChange` and RadioGroup's `value` / `onValueChange`. |
| `mergeRefs` | Compose Floating UI's setReference + local ref + forwarded ref + root's trigger ref. |

Menu **does not** use `useFocusTrap` — focus stays on the Content element, items are
keyboard-highlighted (not tab-focused), and Tab closes the menu. This matches the platform menu
convention and is what the W3C APG specifies for the Menu role.

`MenuPlacement = PopoverPlacement` — single source of truth for the 12-placement vocab; Modal /
Menu / Select / Combobox all alias from this same canonical type.

### Notes for consumers

A few conventions surfaced during implementation; documented here so the next overlay author
(Select, Combobox) doesn't rediscover them:

- **Keyboard registry is DOM-order, not mount-order** — items register via callback ref + sort by
  `compareDocumentPosition` on each `getEnabledItems()` call. Dynamic / re-ordered item lists
  work correctly without any per-item index prop.
- **Type-ahead buffer is per-`useMenuKeyboard` instance**, not per-component. SubContent gets its
  own buffer, so typing "d" inside a submenu only matches sub items.
- **Highlight ≠ focus** — `data-highlighted="true"` is what drives the visual + screen-reader
  signal. `node.click()` is dispatched on Enter/Space, which lets Items, CheckboxItems, and
  RadioItems all share one selection code path through their `onClick` handler.

## DRY notes

- No `clsx` / `cn` imports — every class composition flows through `useThemedClasses`.
- `useMenuKeyboard` lives locally; promote to `_shared/useListKeyboard.ts` if Select (Phase 23) or
  the future Combobox prove a second consumer wants the same shape.
- `MenuPlacement = PopoverPlacement` — alias for the 12-placement vocab.
- Checkbox / Radio indicators reuse the Phase 9 / Phase 11 visual idiom (`<Check>` icon and
  centered dot) inline rather than re-importing — same shape, no `_shared/` churn.
- Adding a new color = palette entry + 2 compound rows in `content` recipe.
- Adding a new variant = one base row + 7 compound rows.
- Compound assembly (`Object.assign`) follows Popover / Modal / Card verbatim.

## Out of scope (V1)

- **"Triangle path" submenu hover prediction** (Apple-style heuristic) — V2; the basic hover delay
  works fine for now.
- **`Menu.Item` with badge / count slot** — composition; render the badge inside `children`.
- **`Menu.Loading` placeholder** for async-loaded items — defer; an inline spinner works.
- **Native `<select>` substitute** — that's `<Select />` (Phase 23, depends on Menu).
