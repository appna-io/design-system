...
The overflow dropdown is rendered through the same `<Menu>` primitive (Phase 22) that powers
the rest of the DS — no custom dropdown logic lives in Breadcrumbs. Clicking a hidden crumb
...
The plan calls for `<I18nProvider>` integration (translatable `Breadcrumb` and
`Show hidden navigation items` strings). That primitive hasn't shipped yet, so this phase
exposes both strings as overridable props (`aria-label`, `overflowAriaLabel`). When
`<I18nProvider>` lands, the same prop surface will plug into the translation hook with no
breaking changes.
...
- Per-item overflow priority (e.g. always keep crumb #2 visible when collapsing).
- Drag-to-reorder — not a DS concern.
- Animated collapse — overflow opens through Menu's existing animation.
