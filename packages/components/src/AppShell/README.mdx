...
Target: **< 5 KB gz** (excluding `<Drawer>`, which is externalized — it's already in the
bundle if the consumer uses it elsewhere, and AppShell only pays its cost when
`sidebarBreakpoint` actually triggers).
