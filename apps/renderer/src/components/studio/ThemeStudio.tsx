...
/**
 * Theme Studio — a slide-out drawer that exposes the runtime override layer through a tabbed
 * editor (palette, radii, motion) plus a copy-as-code panel. All edits stream into
 * `useThemeOverrides()` and reflect across every component on the page instantly.
 *
 * The drawer is rendered through a portal into `document.body` so that its `position: fixed`
 * is anchored to the viewport — not to the topbar `<header>`, which carries `backdrop-filter`
 * (and therefore creates a containing block for fixed descendants per CSS spec).
 *
 * The renderer pins `<body dir="ltr">` so that the docs chrome never flips with the Direction
 * toggle (that toggle is scoped to the example preview frame). The drawer therefore always
 * lives in an LTR context and uses physical `right-0` / `border-l` and a plain
 * `translate-x-full` slide — we deliberately skip Tailwind's `rtl:` variant, because that
 * variant matches via the `[dir="rtl"]` *ancestor selector* and would still fire under
 * `<html dir="rtl">`, flipping the slide direction even though the drawer's effective CSS
 * direction is LTR (and leaving the closed drawer half-visible on screen).
 */
export function ThemeStudio() {
  ...
  // Defer portal mount until after hydration. `createPortal(..., document.body)` would throw on
  // the server; gating on a mount flag keeps SSR markup stable and avoids hydration mismatches.
  useEffect(() => {
    setMounted(true);
  }, []);
