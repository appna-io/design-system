...
The `apx-ds` theming system is built on three orthogonal axes:
...
shadows, …). They're defined in `@apx-ds/tokens/variants` and can be extended by passing a
custom theme to `<ThemeProvider>`:

```tsx
import { ThemeProvider, defineTheme } from 'apx-ds';
